...
for my $window (mercury_retrogrades_for_window($iflag, $opt{start_ts}, $window_end_ts)) {
...
sub usage {
    return <<'USAGE';
Usage:
  perl Mercury-Year-Retrogrades.pl [--ephe-path PATH] [--start-ts UNIX_TIMESTAMP]

Examples:
  perl Mercury-Year-Retrogrades.pl
  perl Mercury-Year-Retrogrades.pl --start-ts 1785283200
