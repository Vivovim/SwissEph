...
for my $month (@months) {
    print_month_report($opt{year}, $month, \%opt);
}
...
sub print_month_report {
    ...
    my $start_epoch = timelocal(0, 0, 0, 1, $month - 1, $year);
    ...
    my $month_label = $start_local->strftime('%B %Y');
    say "=" x 88;
    say "Moon Phase Calendar for $month_label";
    say "Search start ($LOCAL_TZ): " . $start_local->strftime('%Y-%m-%d %H:%M:%S');
    say sprintf(
...
sub usage {
    return <<'USAGE';
Usage:
  perl moon_phase_calendar.pl --lat 47.3769 --lon 8.5417 [options]

Options:
  --debug              Print ephemeris path checks and exact phase diagnostics.
  --month N            Limit output to one month (1-12). Default: all months.
  --year YYYY          Year to report. Default: current local year.
...
Notes:
  The search anchor is the first day of each selected month in America/Phoenix.
